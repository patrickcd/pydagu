...
# Test configuration
MAX_EXAMPLES = 5
MAX_HYPOTHESIS_EXAMPLES = 3  # Keep low - hypothesis-jsonschema struggles with complex anyOf
...
def cron_field(draw, min_val, max_val, allow_names=None):
    ...
    if allow_names:
        ...
        def named_range(draw):
            names = draw(st.lists(st.sampled_from(allow_names), min_size=2, max_size=2, unique=True))
...
def cron_expression(draw, include_year=False):
    ...
    month = draw(cron_field(1, 12, allow_names=['JAN', 'FEB', 'MAR', 'APR', 'MAY', 'JUN', 
                                                  'JUL', 'AUG', 'SEP', 'OCT', 'NOV', 'DEC']))
    weekday = draw(cron_field(0, 6, allow_names=['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT']))
...
def test_generated_dag_validation(iteration, yaml_file):
    ...
    dag_configs = [
        {
            ...
            "steps": [
                f"echo 'Step {i}'" for i in range((iteration % 3) + 1)
            ],
    ...
    try:
        ...
        pytest.fail(f"Failed to create DAG model: {e}\nConfig: {json.dumps(config, indent=2)}")
    ...
    print(f"✓ DAG validated successfully with dagu")


@pytest.mark.slow
@given(dag_data=from_schema(Dag.model_json_schema()), cron=st.one_of(st.none(), cron_expression()))
@settings(
...
def test_hypothesis_based_dag_generation(dag_data, cron, yaml_file):
    ...
    if not is_valid:
        ...
        dag_yaml = yaml.dump(dag.model_dump(exclude_none=True), default_flow_style=False)
        print(f"\nFailed DAG content:\n{dag_yaml}")
    ...
    print(f"✓ Hypothesis-generated DAG validated successfully")
...
def test_dagu_cli_available():
    ...
    try:
        ...
        assert result.returncode == 0 or "dagu" in result.stdout.lower() or "dagu" in result.stderr.lower(), \
